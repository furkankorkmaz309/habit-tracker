...
func Signup(app app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(usernameValidationErrors) > 0 {
			...
			handlers.ResponseError(w, errStrUser, http.StatusBadRequest)
		...
		if len(passwordValidationErrors) > 0 {
			...
			handlers.ResponseError(w, errStrPassword, http.StatusBadRequest)
			isErrorOccurred = true
			return
		}
		if len(emailValidationErrors) > 0 {
			...
			handlers.ResponseError(w, errStrEmail, http.StatusBadRequest)
			isErrorOccurred = true
			return
		}
		if isErrorOccurred {
		...
		if costStr == "" {
			...
			handlers.ResponseError(w, "Server configuration error", http.StatusInternalServerError)
			return
		}
